...
APP_HOME="/opt/speech-to-text-service-uni-erfurt"
APP_DATA_LOCATION="/var/opt/speech-to-text-service"
APP_STORAGE_LOCATION="$APP_DATA_LOCATION/storage"
APP_SHARED_STORAGE_LOCATION="$APP_DATA_LOCATION/shared-storage"
CONFIG_FILE_PATH="/etc/opt/speech-to-text-service/speech-to-text-service.yaml"
LOGBACK_CONFIG_FILE_PATH="/etc/opt/speech-to-text-service/logback.xml"

# Container storage locations are fixed and intentionally ignore env overrides.
AUDIO_FILE_STORAGE_LOCATION="$APP_STORAGE_LOCATION/audio"
SHARED_AUDIO_FILE_STORAGE_LOCATION="$APP_SHARED_STORAGE_LOCATION/audio"
RESULT_STORAGE_LOCATION="$APP_STORAGE_LOCATION/results"
SHARED_RESULT_STORAGE_LOCATION="$APP_SHARED_STORAGE_LOCATION/results"

DB_SQL_FILE_PATH="$APP_STORAGE_LOCATION/speech-to-text-db.sqlite"
...
require_file LOGBACK_CONFIGURATION_FILE_LOCATION "$LOGBACK_CONFIG_FILE_PATH"
...
mkdir -p \
  ...
  "$APP_SHARED_STORAGE_LOCATION" \
  "$AUDIO_FILE_STORAGE_LOCATION" \
  "$SHARED_AUDIO_FILE_STORAGE_LOCATION" \
  "$RESULT_STORAGE_LOCATION" \
  "$SHARED_RESULT_STORAGE_LOCATION"
...
exec java \
  -Dlogback.configurationFile="$LOGBACK_CONFIG_FILE_PATH" \
  -jar "$APP_HOME/speech-to-text-service.jar" \
  "$CONFIG_FILE_PATH"
